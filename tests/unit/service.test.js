  ...
  describe("getServiceById function", () => {
    ...
    describe("given service's id is not found", () => {
        it("should not allow to get an inexistent service", async () => {
            ...
            jest.spyOn(serviceRepository, "findServiceById").mockRejectedValueOnce( new ServiceNotFoundError("id", serviceInvalidId));
            ...
            expect(result).rejects.toHaveProperty("message", `Service with id: ${serviceInvalidId} not found`);
  ...
  describe("getServiceByName function", () => {
    describe("given the service's name is valid", () => {
        it("should return the service's data", async () => {
            ...
            const validServiceWithId = createValidServiceWithId({ id: serviceGeneratedId, name: serviceGeneratedName});
    ...
    describe("given service's name is not found", () => {
        it("should not allow to get an inexistent service", async () => {
            ...
            jest.spyOn(serviceRepository, "findServiceByName").mockRejectedValueOnce( new ServiceNotFoundError("name", serviceInvalidName));
            const result = servicesService.getServiceByName(serviceInvalidName);

            expect(result).rejects.toHaveProperty("message", `Service with name: ${serviceInvalidName} not found`);
  ...
  describe("getAllServices function", () => {
    describe("given the getAllServices is called", () => {
        it("should return all services created", async () => {
            ...
            jest.spyOn(serviceRepository, "getAllServices").mockImplementationOnce(() => [validServiceWithId, validServiceWithId2]);
  ...
  describe("updateService function", () => {
    ...
    describe("given the service's id provided is not found", () => {
        it("should not allow the update", async () => {
            ...
            jest.spyOn(serviceRepository, "findServiceById").mockRejectedValueOnce( new ServiceNotFoundError("id", serviceInvalidId));
            const result = servicesService.updateService({ id: serviceInvalidId, name: validServiceBody });
    ...
    describe("given the service's data is invalid", () => {
        it("should not allow the update", async () => {
            ...
            expect(result).rejects.toHaveProperty("message", `Already exists a service with this data!`);
